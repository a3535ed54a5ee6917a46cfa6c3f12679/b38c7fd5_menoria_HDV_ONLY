...
import com.menoria.auctions.gui.elements.Element;
...
public class GuiScreen extends Gui
{
    ...
    //Add
    private @Getter List<Element> elements = Lists.newArrayList();
    private @Getter ElementButton current;
    ...
    public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_)
    {
        ...
        for(int count = 0; count < this.elements.size(); count++) {
        	...
        	Element button = this.elements.get(count);
        	button.display(p_73863_1_, p_73863_2_);
        }
    }
	...
	public void drawTexturedModalRectWithOptionnalSize(int x, int y, int u, int v, int width, int height) {
		...
		tessellator.addVertexWithUV((double) (x + 0), (double) (y + height), (double) zLevel, (double) ((float) (u + 0) * scaledX), (double) ((float) (v + height) * scaledY));
		tessellator.addVertexWithUV((double) (x + width), (double) (y + height), (double) zLevel, (double) ((float) (u + width) * scaledX), (double) ((float) (v + height) * scaledY));
		tessellator.addVertexWithUV((double) (x + width), (double) (y + 0), (double) zLevel, (double) ((float) (u + width) * scaledX), (double) ((float) (v + 0) * scaledY));
		tessellator.addVertexWithUV((double) (x + 0), (double) (y + 0), (double) zLevel, (double) ((float) (u + 0) * scaledX), (double) ((float) (v + 0) * scaledY));
		tessellator.draw();
	}

	public void renderLogo(int posWidth, int posHeight , int width, int height) {
		
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		GL11.glDisable(GL11.GL_ALPHA_TEST);
		GL11.glEnable(GL11.GL_BLEND);
		this.mc.getTextureManager().bindTexture(new ResourceLocation("textures/gui/logo_meno.png"));
		this.drawTexturedModalRectWithOptionnalSize(posWidth, posHeight, 0, 0, width, height);
		GL11.glEnable(GL11.GL_BLEND);
	...
	public void drawText(String text, int x, int y) {
		this.mc.fontRenderer.drawStringWithShadow(EnumChatFormatting.color(text), x, y, Colors.GRAY.getDisableColor());
	}
        ...
        if (p_73869_2_ == 1)
        {
        	...
        	for (Element element : this.elements) {
    			if (element instanceof ElementTextBox) {
    				...
    				if (textBox.getTextField().getText().equalsIgnoreCase(textBox.getDefaultText())) {
    					textBox.getTextField().setText("");
    				}

    				textBox.getTextField().textboxKeyTyped(p_73869_1_, p_73869_2_);
    				if (textBox.getOnTextUpdate() != null) {
    					textBox.getOnTextUpdate().onTextUpdate(textBox.getTextField().getText());
    				}
    			}
    		}
        }
    ...
    protected void mouseClicked(int p_73864_1_, int p_73864_2_, int p_73864_3_)
    {
        if (p_73864_3_ == 0)
        {
            ...
            //Add
            for(int count = 0; count < this.elements.size(); count++) {
            	Element element = this.elements.get(count);
            	
            	if (element instanceof ElementTextBox) {
					ElementTextBox textBox = (ElementTextBox) element;
					textBox.getTextField().mouseClicked(p_73864_1_, p_73864_2_, p_73864_3_);
				} else if (element instanceof ElementButton) {
					ElementButton button = (ElementButton) element;
					if (button.isPressed(p_73864_1_, p_73864_2_)) {
	            		this.current = button;
    ...
    protected void mouseMovedOrUp(int p_146286_1_, int p_146286_2_, int p_146286_3_)
    {
        ...
        if(this.current != null  && p_146286_3_ == 0) {  	
        	this.current = null;
        }
    }
    ...
    public void updateScreen() {
		for (Element element : this.elements) {
			if (element instanceof ElementTextBox) {
				ElementTextBox textBox = (ElementTextBox) element;
				textBox.getTextField().updateCursorCounter();
			}
		}
    }
